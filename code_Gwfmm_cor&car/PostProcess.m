function [betahat,beta_025CI,beta_975CI,alphahat,accept_rate_theta,accept_rate_rho,ghat,ghatns,Q025_ghat,Q975_ghat,thetahat,rhohat, theta_025CI,theta_975CI,tauhat,pihat,...
...
MCMC_rho = dlmread(strcat(paramroute,'/MCMC_rho_rec.txt'));
thetahat=reshape(mean(MCMC_theta),size(theta,1),size(theta,2));
rhohat = mean(MCMC_rho);

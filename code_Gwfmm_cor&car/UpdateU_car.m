function U=UpdateU_car(Rinv,beta,theta,model,D,wavespecs)
...
U=NaN(M,wavespecs.K);
for j=1:wavespecs.K
   %Mjk=diag(model.C*theta(model.H+1:model.H+model.c,j).^(-1)); %zhu2012: just need to change this step.
   D_neghalf=diag(model.C*theta(model.H+1:model.H+model.c,j).^(-1/2));
   Mjk=D_neghalf*Rinv(:,:,j)*D_neghalf;
   ...
   A=[A11,A12;A12',A22];          
   [~, i_sort]=sort(diag(A11)); %%%% sweep largest diagonals first    

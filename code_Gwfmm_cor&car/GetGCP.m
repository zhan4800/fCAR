...
for l=1:c        %#zhu#% When c!=1, extract that group of data and compute them separately, this may help remove unnecessary columns of X or Z, save time.
 ...
 for j=1:K        
      ...
      if temp_indicator==1
            ...
            if c==1
                L1(j)=sum(log(piv))+n*log(theta_s(j)); % zhu2012: refer:page 1298 Wolfinger. log|V|=sum (log(piv))+log|R*s_{jk}|. When R is not I, this line should change.
                ...
                % zhu2012: this should change too when R is not I.
            end
            ...
            L1(j)=sum(C(:,1))*log(theta_s(1,j)); %zhu2012: this should be updated when R is not I, log|Sigma_jk|=log|R*s_jk|=sum(C(:,1))*log(theta_s(1,j))+log|R|.
            if c>1
                for l=2:c
                    ...
                    L1(j)=L1(j)+sum(C(:,l))*log(theta_s(l,j));%zhu2012: when R is not I, should update here as in line 150.
                end
            end
      end
 end % j for loop

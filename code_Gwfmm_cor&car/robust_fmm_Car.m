function res = robust_fmm_Car(Y,model,wavespecs,MCMCspecs,sampleU,get_sigma,blocksize,paramroute,auto) 
%function res = wavfmm4_v5_zhu(D,model,wavespecs,MCMCspecs,sampleU,get_sigma,blocksize,paramroute) 
%==========================================================================
% Note: robust_fmm_Car.m is based on Gfmm_Cor.m and wavfmm8_v2_zhu.m.
% The function performs robust functional mixed model analysis with the spatial
% correlation among the functional samples accounted by a CAR model.
% The CAR model is incorporated in the R matrix, which is an N by N matrix,
% taking the form as R=(Dv-rho*V)^-1, where V is a user defined
% neighborhood matrix, and rho is referred to as "spatical correlation"
% parameter (Wall, 2004).
% 
%==========================================================================
...
%   auto       = a structure contains the the initial value for the CAR
%                model-based autocorrelation matrix, R. See function autocor_car.m for details. 
%                It incorporates three types of the functional CAR model
%                bay the optional value of auto.type: 
%                (1) 'separable', which assumes a common spatial parameter
%                rho for all wavelet basis; (2) 'change.over.j', which assumes 
%                the spatial parameter rho changes over j; (3) 'change.over.jk', 
%                which assumes that wavelet component jk has a different
%                rho value. This allows different levels of flexibility in
%                the spatial correlations among the samples for different
%                wavelet components.
%
...
[tau,nu2,PI,alpha,a_nu2,b_nu2,a_pi,b_pi] = initial_values(betans,Vbetans,model,wavespecs,MCMCspecs,meanop);
initial.PI = PI;
initial.tau = tau;
initial.nu2 = nu2;
initial.a_nu2 = a_nu2;
...
TauMat = tau./Vbetans; %#zhu#%  In this code, do we always assume covT=1?
...
MCMC_tau=NaN(blocksize,p*wavespecs.K); % in the memory, we only save a block of the samples. 
MCMC_nu2=NaN(blocksize,p*wavespecs.J);
...
for i=1:(B*thin+burnin)
    ...
    [theta,flag_theta,Vbetans,Wv] = UpdateTheta1_car(beta,theta,Vbetans,Wv,D,W,model,prior_Theta_a,prior_Theta_b,propsd_Theta,wavespecs,MCMCspecs,Rivmat);
    
    % (3) Update R.         
    %[auto, Vbetans, Wv, W, Rivmat,nreject] = UpdateR_MH_car(auto, beta, theta, Vbetans, Wv, W, D, model, wavespecs, MCMCspecs, Rivmat);    
    nreject = 0;
    ...
    [tau,nu2]=Update_tau_nu2(beta,gamma,tau,nu2,a_nu2,b_nu2,MCMCspecs.minVC,wavespecs);
    TauMat=tau./Vbetans;    
    ...
    if   (i>burnin)&&(mod(i-burnin,thin)==0)   %% Save MCMC samples of beta in single matrix.
            ...
            MCMC_tau(is,:)=reshape(tau,1,p*wavespecs.K);    % each block is one jk, contains p values.
            MCMC_nu2(is,:)=reshape(nu2,1,p*wavespecs.J);    % each block is one j, contains p values.

function [betans,Vbetans,Wv]=GetGCP_cor1(theta,W,Rinv,model,wavespecs,Update_betans,D,MCMCspecs)
...
for l=1:c        %#zhu#% When c!=1, extract that group of data and compute them separately, this may help remove unnecessary columns of X or Z, save time.
    ...
    temp_W{l} = GetW_cor1(temp_model,temp_D,Rinv);
end
 ...
 for j=1:K        
      ...
      if temp_indicator==1
            ...
            if c==1
                L1(j)=sum(log(piv))+n*log(theta_s(j))-log(det(Rinv)); % zhu2012: refer:page 1298 Wolfinger. log|V|=sum (log(piv))+log|R*s_{jk}|. When R is not I, this line should change.
                ...
                L1(j)=sum(log(piv))+sum(log(model.C*theta_s(:,j)))-log(det(Rinv)); %#zhu#% When c>1, the R matrix in sweep operator is not homogeneous in diagonal.
                % zhu2012: -log(det(Rinv)) was added for the
            ...
            L1(j)=sum(C(:,1))*log(theta_s(1,j))-log(det(Rinv(logical(C(:,1)),logical(C(:,1))))); %zhu2012: this should be updated when R is not I, log|Sigma_jk|=log|R*s_jk|=sum(C(:,1))*log(theta_s(1,j))+log|R|.
            ...
            if c>1
                for l=2:c
                    ...
                    L1(j)=L1(j)+sum(C(:,l))*log(theta_s(l,j))-log(det(Rinv(logical(C(:,l)),logical(C(:,l)))));
                    %zhu2012: when R is not I, the third term come into
                    %play. again, similar assumption was made as line 152.
                end                 
            % zhu2012: line 158, when c>1, we break down E_{jk} according to
            % model.C to several independent parts. We here assume that if this
            % happens, then the autocorrelation matrix can also be break
            % down to two several diagonal parts.
            end           
      end
 end % j for loop
